...
// Buscar reservas dentro de los próximos 70 minutos (margen) que aún no tengan recordatorio.
// Usamos 70 en lugar de 60 para tolerancia si el cron se ejecuta cada 10 minutos.
$sql = "
...
foreach ($rows as $row) {
    try {
        ...
        <div class="header"><h2>⏰ Recordatorio de Clase</h2></div>
        <div class="content">
        <p>Hola <strong>' . htmlspecialchars($studentName) . '</strong>, tu <strong>' . htmlspecialchars($tipo) . '</strong> comienza en aproximadamente <strong>1 hora</strong>.</p>
        <div class="info-box">
        ...
        ' . ($meetingLink ? '<p style="text-align:center"><a href="' . htmlspecialchars($meetingLink) . '" class="button">🔗 Unirse a la clase</a></p>' : '<p style="background:#fff3cd;padding:12px;border-radius:6px;color:#856404">El enlace de la videollamada será agregado por el profesor.</p>') . '
        ...
        <div class="footer">Este es un correo automático. &copy; ' . date('Y') . ' Atisbe Academy.</div>
        </div></div></body></html>';
...
// Accion: Program/Script: php.exe
// Argumentos: C:\ruta\a\public\api\schedule\reminders_cron.php
// Iniciar en: C:\ruta\a\(directorio del proyecto)
?>